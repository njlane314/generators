...
if [[ $# -gt 7 ]]; then
  cat >&2 <<'EOF'
Usage: analysis/run_enu_q2_coverage.sh [INPUT_SOURCE] [OUTPUT_DIR] [LABELS_OR_GROUPINGS] [OUTPUT_STEM] [SELECTION] [PROTON_THRESHOLD] [PIMINUS_THRESHOLD]
...
root -l -b -q "analysis/plot_enu_q2_coverage.cxx+(\"${input_source}\",\"${output_dir}\",\"${labels_or_groupings}\",\"${output_stem}\",\"${selection}\",${proton_threshold},${piminus_threshold})"
