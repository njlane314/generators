...
generation_fluxfile="${generation_fluxfile:-${reweight_fluxfile:-$(ana_flux_root "${repo_root}")}}"
generation_fluxhisto="${generation_fluxhisto:-${reweight_fluxhisto:-$(ana_flux_hist "${beam_mode}" "${beam_species}")}}"
...
case "${generation_flux_mode}" in
  ...
  flat)
    ...
    prepare_flux="${prepare_flux:-${proposal_min_gev},${proposal_max_gev},${proposal_bin_width_gev}}"
    proposal_label="flat ${proposal_min_gev}-${proposal_max_gev} GeV, bin ${proposal_bin_width_gev} GeV"
    ana_check_files "${spline}"
    ;;
...
if [ "${generation_flux_mode}" = "flat" ]; then
  gevgen \
    -n "${events}" \
    -p "${probe}" \
    -t "${target}" \
