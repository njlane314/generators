...
std::vector<plan_row> read_plan(const TString& plan_file)
{
    ...
    if (!input) {
        std::cerr << "Could not open sample matrix: " << plan_file.Data() << std::endl;
        return rows;
    }
...
{
    load_macro_once("analysis/libexec/root/matrix/primary_mechanism.cxx", loaded);
...
{
    load_macro_once("analysis/libexec/root/matrix/nuclear_exit.cxx", loaded);
    const TString beam_polarity = beam_polarity_from_mode(row.beam_mode);
...
void run_generator_matrix(
    TString plan_file = "analysis/share/config/sample_matrix.tsv",
