...
void style_hist(TH1* hist, int colour, int marker)
{
    hist->SetStats(false);
...
{
    const std::vector<int> colours = {kBlue + 1, kRed + 1, kGreen + 2, kMagenta + 1, kOrange + 7,
    ...
    for (const status_row& row : rows) {
        ...
        if (!hist) continue;
        if (spec.normalise) normalise_shape(hist);
        style_hist(hist, colours[hists.size() % colours.size()], 20 + (hists.size() % 10));
        hist->GetYaxis()->SetTitle(spec.normalise ? "Normalised events" : "Weighted events");
