...
std::vector<StatusRow> read_status(const TString& status_csv)
{
    ...
    while (std::getline(input, line)) {
        ...
        row.knob = f[4].c_str();
        row.beam_mode = normalise_beam_mode(f[5].c_str());
        row.beam_species = normalise_beam_species(f[6].c_str());
...
TGraphAsymmErrors* make_band(TH1D* a, TH1D* b)
{
    ...
    for (int bin = 1; bin <= a->GetNbinsX(); ++bin) {
        ...
        band->SetPoint(bin - 1, x, centre);
        band->SetPointError(bin - 1, ex, ex, centre - low, high - centre);
    }
    return band;
}

void style_hist(TH1D* hist, int colour)
{
    hist->SetStats(false);
