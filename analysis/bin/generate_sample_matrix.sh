...
run_generator_sample() {
  ...
  case "${generator}" in
    GENIE)
      ...
      proposal_min_gev="${beam_energy_min_gev}" \
      proposal_max_gev="${beam_energy_max_gev}" \
      run="$((seed_base + n))" \
      sample="${sample_label}" \
      outdir="${outdir}" \
      workdir="${workdir}" \
