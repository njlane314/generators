...
if [[ $# -gt 7 ]]; then
  cat >&2 <<'EOF'
Usage: analysis/bin/plot_generator_kinematic_envelope.sh [STATUS_CSV] [OUTPUT_DIR] [VARIABLES] [SELECTION] [PROTON_THRESHOLD] [PIMINUS_THRESHOLD] [FIX_FSI]
...
ana_cd_repo
root -l -b -q "${ANA_ROOT_DIR}/envelopes/generator_kinematic_envelope.cxx+(\"${status_csv}\",\"${output_dir}\",\"${variables}\",\"${selection}\",${proton_threshold},${piminus_threshold},${include_fsi_state_in_group})"
