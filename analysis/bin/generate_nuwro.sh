...
sanitise_card="${sanitise_card:-1}"

base_card="${base_card:-${ANA_SHARE_DIR}/cards/NuWroCard_CC_Ar_numu.txt}"
...
outdir="${outdir:-${repo_root}/analysis/output/nuisance_flat/NuWro}"
...
if [ "${sanitise_card}" = 1 ]; then
  awk '
    /^[[:space:]]*(sf_nuclearRecoil|sf_CoulombDistortion|sf_src|sf_pb)[[:space:]]*=/ {
      print "# disabled by analysis/bin/generate_nuwro.sh for installed NuWro compatibility: " $0
      next
    }
    { print }
  ' "${base_card}" > "${run_card}"
