...
if [[ $# -gt 7 ]]; then
  cat >&2 <<'EOF'
Usage: analysis/bin/compute_expected_yields.sh [STATUS_CSV] [OUTPUT_DIR] [EXPOSURE_SCALE] [PROTON_THRESHOLD] [PIMINUS_THRESHOLD] [ENERGY_MIN] [ENERGY_MAX]
...
ana_cd_repo
root -l -b -q "${ANA_ROOT_DIR}/yields/expected_event_yields.cxx+(\"${status_csv}\",\"${output_dir}\",${exposure_scale},${proton_threshold},${piminus_threshold},${energy_min},${energy_max})"
